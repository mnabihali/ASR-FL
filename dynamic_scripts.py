...
trainloaders, valloaders, trainloader_gl, _= load_datasets() 

def train_asr(net, epochs:int, add_train=True):
    net.train()
    epoch_loss = 0
    acc = []
    add_train=False
    if add_train:
        ...
        random_index = random.randrange(len(trainloaders))
        iterator = trainloaders[int(random_index)]
...
def eval_asr(net):  # I need to return the loss also and WER
